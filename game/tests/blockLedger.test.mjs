...
const historyMigration = await readFile(new URL('../../backend/db/migrations/20260914_block_cashout_history.sql', import.meta.url), 'utf8');
...
try {
  ...
  const played = { ...state, moves: 8, total_clears: 5, best_combo: 1 };
  ...
  const fourClears = { ...played, total_clears: 4 };
  await commit(player, 'round-1', 'four-clears', 1, fourClears);
  assert.equal((await commit(player, 'round-1', 'too-soon', 2, fourClears, 'won', 7440)).reason, 'invalid', 'four lines cannot unlock cashout');
  await commit(player, 'round-1', 'five-clears', 2, played);
  const won = await commit(player, 'round-1', 'cashout-1', 3, played, 'won', 8400);
  assert.equal(won.round.payout, 84);
  assert.equal(won.balance, 154);
  assert.equal((await read(player)).balance, 154, 'resume reads the settled wallet');
  assert.equal((await read(player)).round.status, 'won');
  assert.equal((await read(other)).reason, 'not_found', 'resume enforces ownership');
  assert.equal((await commit(player, 'round-1', 'cashout-1', 1, played, 'won', 8400)).balance, 154);
  assert.equal((await commit(player, 'round-1', 'second-cashout', 1, played, 'won', 8400)).reason, 'conflict');
  ...
  assert.deepEqual(ledger.map(row => row.amount), [-30, 84]);
  const demoId = '00000000-0000-4000-8000-000000000003';
  for (let attempt = 0; attempt < 2; attempt++) {
    await query("INSERT INTO block_demo_rounds(round_id,user_id,bet,status,moves,total_clears,best_combo,multiplier,payout) VALUES ($1,$2,30,'lost',8,2,1,1.84,0) ON CONFLICT (round_id) DO NOTHING", [demoId, player]);
  }
  assert.equal(await balance(player), 154, 'demo history never changes the wallet');
  assert.equal((await query('SELECT count(*)::integer AS count FROM wallet_transactions WHERE user_id=$1', [player]))[0].count, 2);
  const lobby = (await query('SELECT read_lobby_snapshot($1) AS data', [player]))[0].data;
  assert.equal(lobby.user.balance, 154);
  assert.equal(lobby.user.password_hash, undefined);
  assert.equal(lobby.rounds.filter(row => row.demo_mode).length, 1, 'retry creates one demo record');
  assert.equal(lobby.rounds.find(row => row.round_id === 'round-1').payout, 84);
  assert.equal((await query('SELECT read_lobby_snapshot($1) AS data', [other]))[0].data.rounds.length, 0, 'history is private to the account');
  ...
  assert.equal(await balance(player), 124);
  ...
  await assert.rejects(commit(player, 'payout-rollback', 'cashout', 1, played, 'won', 8400), /settlement failure/);
  assert.equal(await balance(player), 94, 'failed settlement cannot credit the wallet');
  ...
  for (const role of ['anon', 'authenticated']) {
    ...
    assert.equal((await query("SELECT has_function_privilege($1,'public.read_lobby_snapshot(text)','EXECUTE') AS read", [role]))[0].read, false);
    assert.equal((await query("SELECT has_table_privilege($1,'public.block_demo_rounds','SELECT,INSERT,UPDATE,DELETE') AS allowed", [role]))[0].allowed, false);
